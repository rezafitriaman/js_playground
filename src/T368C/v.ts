...
var checkEurOrDollar = (previous, now, page) => {
...
//PDP funtion
var pdp = (target, currency):void => {
	var button: HTMLCollection = document.getElementsByClassName('productDetail-addToBagLink');
	var prevEl: HTMLCollection = document.getElementsByClassName('productPrice-discount');
	var nowEl: HTMLCollection = document.getElementsByClassName('productPrice-value');

	//if there are discount
	if(prevEl.length != 0) {
		var previous = (<any>prevEl[0]).innerHTML
		var now = (<any>nowEl[0]).innerHTML
		...
		var placeHolder: HTMLCollection = document.getElementsByClassName('container-t4u');

		(<HTMLElement>placeHolder[0]).innerText = target[language];
